...
sourceISO="/tmp/autoinatall.iso"
resultISO="$(pwd)/result.iso"
buildDir="/tmp/autoinstall-ISO/"
sourceIsoPath="/tmp/autoinstall-ISO/autoinatall.iso"
...
if [ ! -f "$sourceISO" ]; then
  wget https://mirrors.jxust.edu.cn/ubuntu-releases/24.04.3/ubuntu-24.04.3-live-server-amd64.iso -O $sourceISO
...
sed -i "/${insertPoint}/i \ \ \ \ linux   /casper/vmlinuz quiet autoinstall ds=nocloud\\\;s=/cdrom/server/  ---" $grubPath
sed -i "/${insertPoint}/i \ \ \ \ initrd  /casper/initrd" $grubPath
sed -i "/${insertPoint}/i }" $grubPath
sed -i 's/timeout=30/timeout=1/g' $grubPath

mkdir $cdromServerPath
touch $metaDataPath
cp -f "user-data" $userDataPath

#!/bin/bash
set -eo pipefail
if [ ! -e "$TF_VAR_libvirt_disk_path" ]; then
  echo "$TF_VAR_libvirt_disk_path does not exist."
  echo "sudo mkdir -p $TF_VAR_libvirt_disk_path"
  exit 1
fi
if [ ! -e "$TF_VAR_cloudimg_url" ]; then
  echo "$TF_VAR_cloudimg_url does not exist."
  echo "sudo mkdir -p $TF_VAR_cloudimg_url && sudo wget https://cloud-images.ubuntu.com/releases/jammy/release/ubuntu-22.04-server-cloudimg-amd64.img  $TF_VAR_cloudimg_url/"
  exit 1
fi
if [ -z "$TF_VAR_libvirt" ]; then
  ...
  echo "ssh-keygen -t rsa && ssh-copy-id -i ~/.ssh/id_rsa_pub user@ip"
